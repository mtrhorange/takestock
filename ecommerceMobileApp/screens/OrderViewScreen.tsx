...
const OrderViewScreen = () => {
  ...
  const filteredOrders =
    selectedTab === "ALL"
      ? orders
      : orders.filter((order) => {
          const tabStatus = orderTabs.find(
            (tab) => tab.status === selectedTab
          )?.status;
        ...
        filteredOrders.map((order) => (
          <Card key={order.orderDTO.id} style={styles.orderCard}>
            <Text style={styles.statusText}>
              {getStatusLabel(order.orderDTO.orderStatus)}
            </Text>
            ...
            <View style={styles.bottomRow}>
              <Text style={styles.totalPrice}>
                Total: ${order.orderDTO.totalPrice.toFixed(2)}
              </Text>
              <View style={styles.actions}>
                {order.orderDTO.orderStatus === "PENDING" && (
                  <Button
                    mode="contained"
                    textColor="white"
                    onPress={() => handleCancelOrder(order.orderDTO.id)}
                    style={styles.actionBtn}
                  >
                    Cancel
                  </Button>
                )}
                {["SHIPPED", "TO_RECEIVE", "PROCESSING", "PAID"].includes(
                  order.orderDTO.orderStatus
                ) && (
                  <Button
                    mode="contained"
                    onPress={() => handleRefundOrder(order.orderDTO.id)}
                    style={styles.actionBtn}
                  >
                    Refund
                  </Button>
